...
def scrape_table(url):
    ...
    for row in table_data[1:]:
        incidente = {'Producto': 'android', 'Vuln ID': row[0], 'Resumen': row[1], 'Gravedad de CVSS': row[2]}
...
url = 'https://nvd.nist.gov/vuln/search/results?form_type=Basic&results_type=overview&query=android&search_type=all&isCpeNameSearch=false'
nombre_archivo_registro = 'vuls/android/registro.json'

fecha_actual = datetime.datetime.now()
nombre_archivo_nuevas_entradas = f"vuls/android/{fecha_actual.strftime('%Y-%m-%d_%H-%M-%S')}_android.json"
