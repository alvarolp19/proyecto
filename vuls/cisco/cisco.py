...
def scrape_table(url):
    ...
    for row in table_data[1:]:
        incidente = {'Producto': 'cisco', 'Vuln ID': row[0], 'Resumen': row[1], 'Gravedad de CVSS': row[2]}
...
url = 'https://nvd.nist.gov/vuln/search/results?form_type=Basic&results_type=overview&query=cisco&search_type=all&isCpeNameSearch=false'
nombre_archivo_registro = 'vuls/cisco/registro.json'

fecha_actual = datetime.datetime.now()
nombre_archivo_nuevas_entradas = f"vuls/cisco/{fecha_actual.strftime('%Y-%m-%d_%H-%M-%S')}_cisco.json"
